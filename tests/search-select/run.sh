...
$DEENURP search_sequences ground-reads.fa output/search.db $RDP_BASE.fasta $RDP_BASE.seqinfo.csv \
	 --group-field=tax_id \
	 --blacklist=exclude_clusters.txt
...
$DEENURP select_references output/search.db output/refs.fasta \
	 ...
	 --min-mass-prop 0.01 \
	 --include-clusters include_clusters.txt
